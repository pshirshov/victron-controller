use crate::victron_controller::dashboard::v0_2_0::soc_history_sample::SocHistorySample;
use crate::victron_controller::dashboard::v0_2_0::soc_projection::SocProjection;
...
pub struct SocChart {
    ...
    pub export_threshold_pct: Option<f64>,
}
...
impl Ord for SocChart {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        match crate::baboon_runtime::__opt_f64_total_cmp(&self.export_threshold_pct, &other.export_threshold_pct) {
            std::cmp::Ordering::Equal => {},
            ord => return ord,
        }
        std::cmp::Ordering::Equal
    ...
    fn encode_ueba(&self, ctx: &crate::baboon_runtime::BaboonCodecContext, writer: &mut dyn std::io::Write) -> std::io::Result<()> {
        ...
        if ctx.use_indices() {
            ...
            }
            {
                let before = buffer.len();
                crate::baboon_runtime::bin_tools::write_i32(writer, before as i32)?;
                match &value.export_threshold_pct {
                None => crate::baboon_runtime::bin_tools::write_byte(&mut buffer, 0)?,
                Some(v) => {
                    crate::baboon_runtime::bin_tools::write_byte(&mut buffer, 1)?;
                    v.encode_ueba(ctx, &mut buffer)?;
