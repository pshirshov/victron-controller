...
pub struct ZappiDrainSnapshotWire {
...
impl PartialEq for ZappiDrainSnapshotWire {
    fn eq(&self, other: &Self) -> bool {
        self.cmp(other) == std::cmp::Ordering::Equal
    }
}

impl Eq for ZappiDrainSnapshotWire {}

impl PartialOrd for ZappiDrainSnapshotWire {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for ZappiDrainSnapshotWire {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        ...
        match self.hard_clamp_excess_w.total_cmp(&other.hard_clamp_excess_w) {
            std::cmp::Ordering::Equal => {},
            ord => return ord,
        }
        match self.threshold_w.cmp(&other.threshold_w) {
            std::cmp::Ordering::Equal => {},
            ord => return ord,
        }
        match self.hard_clamp_w.cmp(&other.hard_clamp_w) {
            std::cmp::Ordering::Equal => {},
            ord => return ord,
        }
...
impl crate::baboon_runtime::BaboonBinCodecIndexed for ZappiDrainSnapshotWire {
    fn index_elements_count(_ctx: &crate::baboon_runtime::BaboonCodecContext) -> u16 {
        0
    }
}

impl crate::baboon_runtime::BaboonBinEncode for ZappiDrainSnapshotWire {
    fn encode_ueba(&self, ctx: &crate::baboon_runtime::BaboonCodecContext, writer: &mut dyn std::io::Write) -> std::io::Result<()> {
        ...
        if ctx.use_indices() {
            ...
            value.hard_clamp_excess_w.encode_ueba(ctx, &mut buffer)?;
            value.threshold_w.encode_ueba(ctx, &mut buffer)?;
            value.hard_clamp_w.encode_ueba(ctx, &mut buffer)?;
            ...
            value.hard_clamp_excess_w.encode_ueba(ctx, writer)?;
            value.threshold_w.encode_ueba(ctx, writer)?;
            value.hard_clamp_w.encode_ueba(ctx, writer)?;
...
impl crate::baboon_runtime::BaboonBinDecode for ZappiDrainSnapshotWire {
    fn decode_ueba(ctx: &crate::baboon_runtime::BaboonCodecContext, reader: &mut dyn std::io::Read) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        let hard_clamp_excess_w = crate::baboon_runtime::bin_tools::read_f64(reader)?;
        let threshold_w = crate::baboon_runtime::bin_tools::read_i32(reader)?;
        let hard_clamp_w = crate::baboon_runtime::bin_tools::read_i32(reader)?;
        let captured_at_epoch_ms = crate::baboon_runtime::bin_tools::read_i64(reader)?;
        Ok(ZappiDrainSnapshotWire {
            compensated_drain_w,
            branch,
            hard_clamp_engaged,
            hard_clamp_excess_w,
